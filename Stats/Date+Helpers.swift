...
public extension Date {
    ...
    static fileprivate var ISO8601TimeZoneFormatter: ISO8601DateFormatter {
        let formatter = ISO8601DateFormatter()
        formatter.timeZone = TimeZone.current
        formatter.formatOptions = [.withFullTime, .withFullTime, .withTimeZone]
        return formatter
    }
    ...
    static public func fromISO8601String(_ dateString: String?) -> Date? {
        ...
        if let date = ISO8601TimeZoneFormatter.date(from: dateString) {
            return date
        }
    ...
    public var isInCurrentYear: Bool {
        return Calendar.current.component(.year, from: self) == currentYear
    }
    ...
    var nextHalfHour: Date {
        let adjustedDate = Calendar.current.date(bySetting: .second, value: 0, of: self)!
        let currentMinutes = Calendar.current.component(.minute, from: adjustedDate)
        
        if currentMinutes < 30, let next30Date = Calendar.current.date(bySetting: .minute, value: 30, of: adjustedDate) {
            return next30Date
        } else if let nextHourDate = Calendar.current.date(byAdding: .minute, value: 60 - currentMinutes, to: adjustedDate) {
            return nextHourDate
        } else {
            return self
        }
